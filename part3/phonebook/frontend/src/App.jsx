...
const App = () => {
    ...
    if (existingContact) {
      ...
      personService
        ...
        .then((savedPerson) => {
          ...
          displayNotification({
            content: `${savedPerson.name} has been updated`,
            type: "added",
          })
        })
        .catch((error) => {
          if (error.response?.status === 404)
            displayNotification({
              content: `${updatedPerson.name} could not be found in the system`,
              type: "error",
            })
        })
    } else {
      ...
      personService
        ...
        .then((response) => {
          ...
          displayNotification({
            content: `${response.name} has been added`,
            type: "added",
          })
        })
        .catch((error) => {
  ...
  const handleDelete = (p) => {
    ...
    personService.deleteContact(p.id).then(() => {
      ...
      displayNotification({
        content: `${p.name} has been deleted`,
        type: "deleted",
      })
    })
  }
